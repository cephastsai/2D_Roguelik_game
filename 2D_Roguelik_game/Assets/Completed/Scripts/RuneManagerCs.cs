...
public class RuneManagerCs : MonoBehaviour {
    ...
    public void random()
    {
        ...
        if (K == 0)
        {
            grid.transform.position = new Vector3(-17, 50, 0);
            aniController.SetInteger("Number", 1);
            StartCoroutine("EnterLevel");
        }
        else if (K == 1)
        {
			grid.transform.position = new Vector3(13, 50, 0);
            aniController.SetInteger("Number", 1);
            StartCoroutine("EnterLevel");
        }
        else if (K == 2)
        {
			grid.transform.position = new Vector3(43, 50, 0);
            aniController.SetInteger("Number", 1);
            StartCoroutine("EnterLevel");
        }
        else if (K == 3)
        {
			grid.transform.position = new Vector3(73, 50, 0);
            aniController.SetInteger("Number", 1);
            StartCoroutine("EnterLevel");
        }
        else if (K == 4)
        {
			grid.transform.position = new Vector3(103, 50, 0);
            aniController.SetInteger("Number", 1);
            StartCoroutine("EnterLevel");
        }
        else if (K == 5)
        {
			grid.transform.position = new Vector3(133, 50, 0);
            aniController.SetInteger("Number", 1);
            StartCoroutine("EnterLevel");
        }
    }
    ...
    void Start () {
		...
		grid.transform.position = new Vector3(0,50,0);
    ...
    void Update()
    {
        ...
        //調整圖騰主要拉軸固定問題
        if (grid.transform.position.x > 163)
        {
            grid.transform.position = new Vector3(163, 50, 0);
        }
